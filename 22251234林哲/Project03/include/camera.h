...
public:
    ...
    void ProcessKeyboardRotate(Camera_Movement direction, float deltaTime)
    {
        ...
        float velocity = MovementSpeed * deltaTime;
        if (direction == FORWARD)
            rot = glm::rotate(rot, velocity * glm::radians(1.0f), glm::vec3(1.0f, 0.0f, 0.0f));
        if (direction == BACKWARD)
            rot = glm::rotate(rot, -velocity * glm::radians(1.0f), glm::vec3(1.0f, 0.0f, 0.0f));
        if (direction == LEFT)
            rot = glm::rotate(rot, -velocity * glm::radians(1.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        if (direction == RIGHT)
            rot = glm::rotate(rot, velocity * glm::radians(1.0f), glm::vec3(0.0f, 1.0f, 0.0f));

        Front = glm::normalize(glm::vec3(0) - Position);
        Position = rot * glm::vec4(Position, 1.0f);
